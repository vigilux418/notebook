...
??? note "`comments.html`"
    ...
    {% if page.meta.comments %}
      ...
      <!-- 在此粘贴 Giscus 配置工具生成的 <script src="https://giscus.app/client.js" ...> -->

      <!-- 同步 Giscus 主题与站点调色板（兼容 navigation.instant 软导航）-->
      <script>
        (function () {
          function giscusTheme() {
            var scheme = document.body.getAttribute("data-md-color-scheme");
            return scheme === "slate" ? "transparent_dark" : "light";
          }
          function syncGiscus() {
            var theme = giscusTheme();
            var loader = document.querySelector("script[src*=giscus]");
          ...
          // ① 本页立即同步一次（首屏 / 硬加载）
          syncGiscus();
          // ② 每次 instant 软导航后再同步；全局仅订阅一次，避免累积
          if (window.document$ && typeof document$.subscribe === "function"
              && !window.__giscusSubscribed) {
            window.__giscusSubscribed = true;
            document$.subscribe(syncGiscus);
          }
          // ③ 调色板切换：事件委托挂到常驻 document，全局仅挂一次，软导航新页也生效
          if (!window.__giscusThemeHooked) {
            window.__giscusThemeHooked = true;
            document.addEventListener("change", function (e) {
              if (e.target && e.target.closest &&
                  e.target.closest("[data-md-component=palette]")) {
                setTimeout(syncGiscus, 0); // 等 Material 写完 body 的 scheme 再读
              }
            });
          }
        })();
    ...
    !!! warning "为什么不用官方文档里的写法"

        Material 官方示例把调色板监听器挂在 `DOMContentLoaded` 里。若你启用了
        `navigation.instant`（即时加载 / 软导航），`DOMContentLoaded` **只在整站第一次
        硬加载时触发一次**，之后站内跳转都不再触发，导致新页面上「切换明暗模式时评论区
        不跟着变、必须按 F5 刷新」。上面的写法改用 Material 自己的 `document$`
        可观察对象（每次软导航后都会触发）+ 事件委托到常驻的 `document`，并用
        `window.__giscus*` 守卫保证只注册一次，从而在 instant 模式下也能实时同步、无需刷新。

- 将上方 `<!-- 在此粘贴 ... -->` 注释替换为你在上一步中使用 [Giscus](https://giscus.app/) 配置工具生成的代码片段。如果您从上面复制了该代码片段，则可以通过将 `comments front matter` 属性设置为 `true` 来在页面上启用评论：
